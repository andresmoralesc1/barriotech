    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white border-b border-stone-200 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-lg font-semibold text-stone-900">Detalle del vendedor</h2>
          <button
            onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-4">
              {vendor.photoUrl ? (
                ...
                <div className="w-16 h-16 rounded-full bg-stone-200 flex items-center justify-center text-stone-400 text-2xl">
                  ?
                </div>
              )}
              <div>
                <h3 className="text-xl font-bold text-stone-900">{vendor.name}</h3>
                <p className="text-sm text-stone-500">{vendor.category ?? 'Sin categoría'}</p>
              </div>
            </div>

            {vendor.description && (
              <p className="text-sm text-stone-700">{vendor.description}</p>
            )}
            ...
            <Section title="Patrocinio">
              {vendor.activeSponsorship ? (
                ...
                <div className="text-sm text-stone-500 italic">Sin patrocinio activo</div>
              )}
            </Section>

            <div className="border-t border-stone-200 pt-6 space-y-3">
              <h4 className="font-semibold text-stone-900 text-sm">Reseñas recientes</h4>
              {vendor.reviewStats.total === 0 ? (
                <div className="text-sm text-stone-500 italic">Sin reseñas todavía.</div>
              ) : (
                <>
                  <div className="flex items-center gap-3 text-sm">
                    <span className="text-stone-500">Distribución:</span>
                    {([5, 4, 3, 2, 1] as const).map((star) => (
                      <span key={star} className="text-xs text-stone-600">
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="text-xs text-stone-500">
                            <span className="font-medium text-stone-700">{rv.authorName}</span>
            ...
            <div className="border-t border-stone-200 pt-6 space-y-3">
              ...
              />
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs text-stone-500">{noteDraft.length} / 2000</span>
                <button
                  onClick={submitNote}
                  ...
                  {notes.map((n) => (
                    ...
                    >
                      <div className="flex justify-between items-start gap-2">
                        <div className="text-xs text-stone-500">
                          <span className="font-medium text-stone-700">{n.author_name}</span>
                          {n.author_email && (
            ...
            <div className="border-t border-stone-200 pt-6 space-y-3">
              ...
              {isDeleted ? (
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={actionPending}
                    className="w-full px-4 py-2 rounded text-sm font-medium bg-stone-100 text-stone-700 hover:bg-stone-200 disabled:opacity-50 border border-stone-300"
                  >
              ...
              <button
                ...
                className="w-full px-4 py-2 rounded text-sm font-medium bg-stone-100 text-stone-700 hover:bg-stone-200"
              >
                Ver página pública
              </button>
            </div>
      ...
      {confirmDelete && vendor && (
        ...
        >
          ...
          >
            <h3 id="confirm-delete-title" className="text-lg font-bold text-stone-900">
              ¿Eliminar vendedor?
            </h3>
            ...
            <div className="mt-5 flex gap-2 justify-end">
              <button
                ...
                className="px-4 py-2 rounded text-sm text-stone-700 hover:bg-stone-100 disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="bg-stone-50 rounded p-3 space-y-1 text-sm">
        {children}
      </div>
    </div>
  )
