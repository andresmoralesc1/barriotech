...
export function DashboardOverview({
  ...
  if (!data) {
    return <div className="p-6 text-center text-sm text-stone-500">Cargando…</div>
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <section>
        <h2 className="text-xs uppercase tracking-wide text-stone-500 font-semibold mb-2">
          Vendedores
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section>
          ...
          <div className="bg-white rounded border border-stone-200 divide-y divide-stone-100">
            ...
            {data.topCities.map((c, i) => (
              <div key={c.cityId} className="px-4 py-2.5 flex justify-between items-center text-sm">
                ...
                <span className="text-stone-900 font-medium">{c.vendorCount}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="bg-white rounded border border-stone-200 max-h-80 overflow-y-auto">
                ...
                <li key={a.id}>
                  <div className="w-full text-left px-4 py-2.5 text-xs hover:bg-blue-50 transition-colors group">
                    <div className="flex justify-between items-baseline">
                      <span className="font-mono text-stone-700 group-hover:text-blue-700">
                        {a.action}
                      </span>
...
function StatCard({
  ...
  const base =
    'bg-white border border-stone-200 rounded px-4 py-3 ' +
    (onClick ? 'cursor-pointer hover:border-stone-400 transition-colors' : '')
