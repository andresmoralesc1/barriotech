/**
 * POST /api/admin/vendors/batch — bulk admin actions on a set of vendors.
 *
 * Body: { ids: string[], action: 'activate' | 'deactivate' | 'verify_email' }
 *
 * - `activate` / `deactivate` flips vendors.is_active.
 * - `verify_email` flips users.email_verified on each vendor's owner.
 *
 * Caps:
 ...
 * commit (a rolled-back tx must leave zero audit). Each vendor gets its
 * own audit row tagged with the batch id so an investigator can group
 * them.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = await parseJsonBody<{ ids?: unknown; action?: unknown }>(req)
  if (!parsed.ok) {
    return NextResponse.json({ error: parsed.error }, { status: 400 })
  }
  const { ids, action } = parsed.body
  ...
  try {
    ...
    try {
      ...
      // Lock the vendor rows we plan to mutate so concurrent batches
      // can't deadlock or act on stale state.
      const lockRes = await client.query(
        `SELECT v.id, v.is_active, p.user_id, u.email_verified
         FROM vendors v
      ...
      if (missing.length > 0) {
        ...
        return NextResponse.json(
          { error: `Vendedores no encontrados: ${missing.join(', ')}` },
          { status: 404 }
        )
      }
      ...
      for (const row of lockRes.rows) {
        ...
        if (action === 'verify_email') {
          await client.query('UPDATE users SET email_verified = $1 WHERE id = $2', [
            next,
            row.user_id,
          ])
        } else {
          await client.query('UPDATE vendors SET is_active = $1 WHERE id = $2', [
            next,
            row.id,
          ])
        }
    ...
    logger.error(serializeErr(err), 'admin vendor batch error')
